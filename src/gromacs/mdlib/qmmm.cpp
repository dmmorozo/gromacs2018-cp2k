...
void
init_gaussian(t_QMrec *qm, t_MMrec *mm);
                  ...
                  matrix box)
{
    ...
    if (qr->QMMMscheme != eQMMMschemeoniom)
    {
        ...
        //copy box
        for (i=0;i<DIM;i++)
           for(j=0;j<DIM;j++) qr->qm[0]->box[i][j]=box[i][j];
                    ...
                    const matrix     box)
{
    ...
    if (qr->QMMMscheme == eQMMMschemenormal) /* also implies 1 QM-layer */
    {
        ...
        //copy box to old box and put new box dimensions
        for (i=0;i<DIM;i++)
           for (j=0;j<DIM;j++)
           {
              qm->oldbox[i][j] = qm->box[i][j];
              qm->box[i][j] = box[i][j];
           }
        ...
        if (QMMMlist->nri)
        {
            ...
            for (i = 0; i < QMMMlist->nri; i++)
            {
                ...
                if (i)
                {
                    qm_i_particles[i].shift = XYZ2IS(0, 0, 0);//pbc_dx_aiuc(&pbc, x[QMMMlist->iinr[0]],
            ...
            //Full MM atoms filling
            crd[0] = IS2X(QMMMlist->shift[0]) + IS2X(qm_i_particles[0].shift);
            crd[1] = IS2Y(QMMMlist->shift[0]) + IS2Y(qm_i_particles[0].shift);
            crd[2] = IS2Z(QMMMlist->shift[0]) + IS2Z(qm_i_particles[0].shift);
            is     = XYZ2IS(0, 0, 0);
            for (j = 0;
                     j < md->nr;
                     j++)
    ...
    *forces2 = nullptr, *fshift2 = nullptr; /* needed for multilayer ONIOM */
    ...
    if (qr->QMMMscheme == eQMMMschemenormal || qr->nrQMlayers == 1)
    {
        ...
        //for (i = 0; i < qm->nrQMatoms + mm->nrMMatoms; i++)
          //	  fprintf(stderr,"atom %.5d  %12.3lf %12.3lf %12.3lf\n", i+1, f[i][0], f[i][1], f[i][2]);
        free(forces);
        free(fshift);
    }
